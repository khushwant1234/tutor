...
const ProfilePage = () => {
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        ...
        setAvatarUrl(
          user.user_metadata?.image_url || user.user_metadata?.avatar_url || ""
        );
      } catch (err) {
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    ...
    try {
      if (isOAuthUser) {
        throw new Error(
          "Password update not available for social login accounts"
        );
      }
        ...
        <Tabs defaultValue="profile" className="max-w-2xl mx-auto">
          <TabsList className="grid w-full grid-cols-2">
            ...
            <TabsTrigger value="security" disabled={isOAuthUser}>
              Security
              {isOAuthUser && (
                <span className="ml-2 text-xs">(Google Account)</span>
              )}
            </TabsTrigger>
          </TabsList>
              ...
              <CardContent className="space-y-6">
                ...
                <div className="border-t border-gray-200 pt-6">
                  ...
                  <form onSubmit={handleAvatarUpload} className="space-y-4">
                    ...
                    <Button type="submit" disabled={!imagePreview || isSaving}>
                      {isSaving ? (
                        <>
                          <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                          Uploading...
